...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load contacts from JSON and populate contact blocks
    fetch('data/contacts.json')
        .then(res => res.ok ? res.json() : Promise.reject(res.status))
        .then(data => {
            const phoneList = document.getElementById('phoneList');
            if (phoneList && Array.isArray(data.phones)) {
                phoneList.innerHTML = data.phones.map(p => `<p><a href="tel:${p.replace(/[^+\d]/g,'')}">${p}</a></p>`).join('\n');
            }
            ...
            if (hoursList && Array.isArray(data.hours)) {
                hoursList.innerHTML = data.hours.map(h => `<p>${h}</p>`).join('\n');
            }
        })
        .catch(err => {
            // silent fail — keep static content if JSON load fails
            console.warn('Не удалось загрузить contacts.json', err);
        });

    // Ensure images lazy-load if browser supports it (fallback handled by JS)
    document.querySelectorAll('img').forEach(img => {
        if (!img.hasAttribute('loading')) img.setAttribute('loading', 'lazy');
    });

    // Ensure videos are muted and don't autoplay audio
    document.querySelectorAll('video').forEach(v => {
        try { v.muted = true; v.preload = 'metadata'; v.playsInline = true; } catch(e){}
    });
});
...
// Форма заказа удалена (заменена на карту). Если понадобятся клиенты для формы,
// можно снова добавить обработчик здесь.